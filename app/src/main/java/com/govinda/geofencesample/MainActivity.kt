...
class MainActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    lateinit var myRepository: ReminderData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myRepository = ReminderData(this)
        ...
        actionNewReminder.setOnClickListener {
            map?.run {
                val intent = Constants.newIntentForReminderActivity(
                    this@MainActivity,
                    cameraPosition.target,
                    cameraPosition.zoom
                )
                startActivityForResult(intent, Constants.REMINDER_REQUEST_CODE)
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == Constants.REMINDER_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            showReminders()
    ...
    private fun onMapAndPermissionReady() {
        if (map != null && ContextCompat.checkSelfPermission(
            ...
            centerCamera()
            actionCurrentLocation.performClick()
        }
    }

    private fun centerCamera() {
        if (intent.extras != null && intent!!.extras!!.containsKey(Constants.LAT_LNG)) {
            val latLng = intent!!.extras!!.get(Constants.LAT_LNG) as LatLng
    ...
    private fun showReminderRemoveAlert(reminderDataModel: ReminderDataModel) {
        ...
        alertDialog.run {
            ...
            setButton(
                ...
                removeReminder(reminderDataModel)
                dialog.dismiss()
            }
            setButton(
    ...
    private fun removeReminder(reminderDataModel: ReminderDataModel) {
        myRepository.remove(reminderDataModel)
        showReminders()
    }
}

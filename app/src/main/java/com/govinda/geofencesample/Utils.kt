...
fun showReminderInMap(context: Context,
  ...
  if (reminderDataModel.latLng != null) {
    val latLng = reminderDataModel.latLng as LatLng
    ...
    marker.tag = reminderDataModel.id
    if (reminderDataModel.radius != null) {
      val radius = reminderDataModel.radius as Double
      map.addCircle(CircleOptions()
          .center(reminderDataModel.latLng)
          .radius(radius)
...
fun sendNotification(context: Context, message: String, latLng: LatLng) {
  ...
  val intent = Constants.newIntentForMainActivity(context.applicationContext, latLng)
